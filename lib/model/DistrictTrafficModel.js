...
export default class DistrictTrafficModel {
  ...
  async init() {
    if (!this._init) {
      ...
      this._initTrafficTrend(
        ...
        overAllSpeedData,
        { trafficFactor: yellowTrafficFactor, deviationFactor: 0.15 }
      );
      // yellow
      ...
      this._initTrafficTrend(
        ...
        overAllSpeedData,
        { trafficFactor: redTrafficFactor, deviationFactor: 0.2 }
      );
      ...
      const restOverallSpeedData =
        overAllSpeedData.map((speed) => {
          ...
          return math.max(30, restAvgSpeed * 1.4);
        });
      // green
      this._initTrafficTrend(
        _.keys(
        ...
        { trafficFactor: 1, deviationFactor: 0.3 });
      this._init = true;
    }
  }
  ...
  async getDistrictsTraffic(time) {
    ...
    return _.mapValues(
      this._districtDict,
      districtTraffic => math.round(districtTraffic.smooth(seconds), 2));
  }

  _initTrafficTrend(districtList, overAllSpeedData, { trafficFactor, deviationFactor }) {
