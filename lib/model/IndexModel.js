...
class BaseIndexModel {
  ...
  getPredictIndexDataModel() {
    const predictSampleDataList = this._sampleDataList.map(
        v => math.max(0, d3.randomNormal(v, 1)()));
...
function _load(asyncInitalizer) {
  let init = false;
  let model = null;
  return async () => {
    if (!init) {
      model = await asyncInitalizer();
      init = true;
    }
    return model;
  };
...
class PredictIndexModelFactory {
  ...
  constructor() {
    this._indexModelFactory = rtIndexModelFactory;
    this._busCountIndexModel = null;
    this._busSpeedIndexModel = null;
    this._overallSpeedIndexModel = null;
    this._pilgrimCountIndexModel = null;
  ...
  async getBusCountIndexModel() {
    if (this._busCountIndexModel == null) {
      const model = await this._indexModelFactory.getBusCountIndexModel();
      this._busCountIndexModel = model.getPredictIndexDataModel();
    }
    return this._busCountIndexModel;
  }
  ...
  async getBusSpeedIndexModel() {
    if (this._busSpeedIndexModel == null) {
      const model = await this._indexModelFactory.getBusSpeedIndexModel();
      this._busSpeedIndexModel = model.getPredictIndexDataModel();
    }
    return this._busSpeedIndexModel;
  }
  ...
  async getOverallSpeedIndexModel() {
    if (this._overallSpeedIndexModel == null) {
      const model = await this._indexModelFactory.getOverallSpeedIndexModel();
      this._overallSpeedIndexModel = model.getPredictIndexDataModel();
    }
    return this._overallSpeedIndexModel;
  }
  ...
  async getPilgrimCountIndexModel() {
    if (this._pilgrimCountIndexModel == null) {
      const model = await this._indexModelFactory.getPilgrimCountIndexModel();
      this._pilgrimCountIndexModel = model.getPredictIndexDataModel();
    }
    return this._pilgrimCountIndexModel;
  }
}

const predictIndexModelFactory = new PredictIndexModelFactory();
