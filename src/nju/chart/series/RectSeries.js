...
export default class RectSeries extends Series
{
  ...
  redraw()
  {
    ...
    this.rect.attr("x", offsetX).attr("y", offsetY).attr("width", width).attr("height", height).style("opacity", this.getOpacity()).style("fill", this.getFill());
  }

  _getSize()
  {
    const parent = this.getParent();
    if (parent && parent.contentFrame && parent.contentFrame.width && parent.contentFrame.height && parent.getScaleX() && parent.getScaleY()) {
